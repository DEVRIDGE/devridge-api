...
public interface CompanyJobRepository extends JpaRepository<CompanyJob, Long> {
    ...
    @Query("select new io.devridge.api.dto.course.CompanyJobInfo(c.name, c.logo, j.name) from CompanyJob cj left join Company c on cj.company.id = c.id left join Job j on cj.job.id = j.id  where cj.company.id = :companyId and cj.job.id = :jobId")
